...
class ImprovedAdaptiveTemporalAttention(nn.Module):
    def __init__(self, in_channels):
        super(ImprovedAdaptiveTemporalAttention, self).__init__()
        self.query_conv = nn.Conv1d(in_channels, in_channels // 16,kernel_size=1)
        self.key_conv = nn.Conv1d(in_channels, in_channels // 16, kernel_size=1)
        self.value_conv = nn.Conv1d(in_channels, in_channels, kernel_size=1)
        self.gamma = nn.Parameter(torch.zeros(1))
        self.channel_reduction = nn.Conv1d(in_channels, 512, kernel_size=1)
...
class Aggregate(nn.Module):
    def __init__(self, len_feature):
        ...
        self.laynorm=LayerNorm(len_feature)
        self.transformer_layer = TransformerEncoderLayer(d_model=len_feature, nhead=8,dropout=0.1)
        self.transformer_encoder = TransformerEncoder(self.transformer_layer, num_layers=1)
        self.attention=ImprovedAdaptiveTemporalAttention(len_feature)
        self.feedword= nn.Sequential(
            LayerNorm(len_feature),
            nn.Conv1d(len_feature, len_feature * 4, 1),
            nn.GELU(),
            nn.Dropout(0.5),
            nn.Conv1d(len_feature * 4, len_feature, 1),

            # nn.Dropout(0.1)

    )
        # self.transf = nn.TransformerDecoderLayer(d_model=len_feature, nhead=8,dropout=0.1)
        # self.tra=nn.TransformerDecoder(self.transf,num_layers=1)
    ...
    def forward(self, x):
        ...
        out_d=out
        residual = out
        # out_d=nn.Conv1d(in_channels=1024,out_channels=512,kernel_size=1)(out)
        out1 = self.conv_1(out)
        out_d=out_d+out1
        out2=self.attention(out)
        out_d=out_d+out2
        out3=self.feedword(out)
        out_d=out_d+out3
        # out1=self.conv_1(out)

        # out_d=nou.Dropout(0.6)(out_d)
        # out_d = out1 + self.gamma1 * out2 + self.gamma2 * out3
        # out2=self.conv_2(out)
        # out3=self.conv_3(out)
        # out_d=torch.cat((out1,out2,out3),dim=1)
        # out1=self.conv_1(out)
        # out_d=self.se(out1)
        out = self.non_local(out1)
